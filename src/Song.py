  ...
  #      else:
    ...
    def getObfuscatedScores(self):
        ...
        for difficulty in self.highScores.keys():
            s[difficulty.id] = [(score, stars, name, self.getScoreHash(difficulty, score, stars, name)) for
                                score, stars, name in self.highScores[difficulty]]
        return binascii.hexlify(Cerealizer.dumps(s).encode())
    ...
    def getDifficulties(self):
        ...
        try:
          ...
          #//  m = midi.MidiInFile()
            m = midi.MidiInFile(info, noteFileName)
            try:
                m.read()
    ...
    def getScoreHash(self, difficulty, score, stars, name):
        s = "%d%d%d%s" % (difficulty.id, score, stars, name)
        s = s.encode()
...
class Song(object):
    def __init__(self, engine, infoFileName, songTrackName, guitarTrackName, rhythmTrackName, noteFileName,
                 scriptFileName=None):
        self.engine = engine
        ...
        if noteFileName:
            ...
            #m = midi.MidiInFile(r, noteFileName)
            m = midi.MidiInFile.MidiInFile(r, noteFileName)
            m.read()
...
class MidiWriter:
    ...
    def write(self):
        ...
        # Collect all events
        events = [zip([difficulty] * len(track.getAllEvents()), track.getAllEvents()) for difficulty, track in
                  enumerate(self.song.tracks)]
    ...
    def note_off(self, channel, note, velocity):
        ...
        try:
            ...
            if note in noteMap:
                ...
                self.addEvent(track, Note(number, endTime - startTime, special=self.velocity[note] == 127),
                              time=startTime)
            else:
